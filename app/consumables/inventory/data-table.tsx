...
export function DataTable<TData, TValue>({
  ...
  const table = useReactTable({
    ...
    globalFilterFn: (row, columnId, filterValue) => {
      ...
      const manufacturer = (row.original as any).manufacturer?.toLowerCase() || ""
      const vendor = (row.original as any).vendor?.toLowerCase() || ""
  ...
  const brandValue = (table.getColumn("brand")?.getFilterValue() as string) ?? "All Brands"
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            ...
            <option value="Toner Laserjet">Toner Laserjet</option>
          ...
          <select
            ...
            onChange={(e) => {
              const val = e.target.value
              table.getColumn("brand")?.setFilterValue(val === "All Brands" ? undefined : val)
            }}
            className="h-10 px-3 rounded-md border border-input bg-background text-xs font-medium text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring"
          >
            <option value="All Brands">All Brands</option>
            <option value="Gaintech">Gaintech</option>
