...
export function DataTable<TData, TValue>({
  ...
  // Since roles is an array, we can filter using custom logic in selector:
  const [roleValue, setRoleValue] = React.useState("All Roles")

  React.useEffect(() => {
    if (roleValue === "All Roles") {
      table.getColumn("roles")?.setFilterValue(undefined)
    } else {
      table.getColumn("roles")?.setFilterValue([roleValue])
    }
  }, [roleValue])
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            ...
            <option value="GA Admin">GA Admin</option>
      ...
      <div className="rounded-xl border bg-card text-card-foreground shadow-xs overflow-hidden">
        ...
        <div className="flex items-center justify-between border-t px-6 py-4">
          <span className="text-xs text-muted-foreground">
            ...
            <span className="font-medium text-foreground">{table.getFilteredRowModel().rows.length}</span> users
          </span>
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
