            ...
            <form action="{{ route('products.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                    <div class="lg:col-span-2">
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" placeholder="e.g., MacBook Pro M3" required
                    ...
                    <div>
                        ...
                        <input type="text" name="product_type" value="{{ old('product_type') }}" placeholder="e.g., A2941"
                            class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800" />
                        ...
                        <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                            <select name="category_id"
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" {{ old('category_id') == $category->id ? 'selected' : '' }} class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                            <select name="master_category_id"
                                ...
                                @foreach($masterCategories as $master)
                                    <option value="{{ $master->id }}" {{ old('master_category_id') == $master->id ? 'selected' : '' }} class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">
                                        {{ $master->name }}
                                    </option>
                                @endforeach
                            </select>
