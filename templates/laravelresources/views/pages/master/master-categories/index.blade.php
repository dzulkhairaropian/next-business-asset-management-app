...
@section('content')
    <div class="space-y-6">
        <x-common.page-breadcrumb pageTitle="Master Categories" />

        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <form action="{{ route('master-categories.index') }}" method="GET" class="flex items-center gap-3">
                ...
                @if(request()->filled('search'))
                    <a href="{{ route('master-categories.index') }}" class="text-sm text-gray-500 hover:text-brand-500">Clear</a>
                @endif
            </form>

            <a href="{{ route('master-categories.create') }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-brand-600">
        ...
        <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]"
            x-data="{ 
                ...
                handleSelectAll() { 
                    ...
                    if (this.selectAll) { 
                        this.selectedRows = {{ json_encode($masterCategories->pluck('id')->toArray()) }}; 
                    } else { 
                        this.selectedRows = []; 
                    } 
                }, 
                handleRowSelect(id) { 
                    ...
                    this.selectAll = this.selectedRows.length === {{ $masterCategories->count() }};
                } 
            }">
            <div class="max-w-full overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-white/[0.05]">
                        ...
                        @endphp
                        @forelse($masterCategories as $item)
                                ...
                                <td class="px-4 sm:px-6 py-3.5">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <span class="block font-medium text-gray-700 text-theme-sm dark:text-gray-400">{{ $item->name }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 sm:px-6 py-3.5">
                                    <span class="block font-medium text-gray-700 text-theme-sm dark:text-gray-400 text-wrap">{{ $item->description }}</span>
                                </td>
                                <td class="px-4 sm:px-6 py-3.5">
                                    <div class="flex items-center gap-3 text-gray-500">
                                        <a href="{{ route('master-categories.edit', $item) }}" class="hover:text-brand-500 transition-colors">
                                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                                        </a>
                                        <form action="{{ route('master-categories.destroy', $item) }}" method="POST" onsubmit="return confirm('Are you sure?')" class="inline-flex">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="px-6 py-10 text-center text-gray-500 italic">No master categories found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
