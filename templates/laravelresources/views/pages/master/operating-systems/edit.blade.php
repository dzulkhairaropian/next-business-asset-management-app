...
@section('content')
    <div class="space-y-6">
        <x-common.page-breadcrumb pageTitle="Edit Operating System" />
            ...
            <form action="{{ route('operating-systems.update', $operatingSystem) }}" method="POST">
                ...
                <div class="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $operatingSystem->name) }}" placeholder="e.g., Windows 11" required
                    ...
                    <div>
                        ...
                        <select name="brand_id" required
                            ...
                            @foreach($brands as $brand)
                                <option value="{{ $brand->id }}" {{ old('brand_id', $operatingSystem->brand_id) == $brand->id ? 'selected' : '' }}>
                                    {{ $brand->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="lg:col-span-2">
                        ...
                        <textarea name="description" rows="3" placeholder="Enter description"
                            class="dark:bg-dark-900 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800">{{ old('description', $operatingSystem->description) }}</textarea>
                    </div>
                </div>
