...
@section('content')
    <div class="space-y-6">
        <x-common.page-breadcrumb pageTitle="Create Status" />
            ...
            <form action="{{ route('statuses.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" placeholder="e.g., Ready to Deploy" required
                    ...
                    <div>
                        ...
                        <input type="text" name="icon" value="{{ old('icon') }}" placeholder="e.g., lucide-check"
                            class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800" />
                    </div>
                    ...
                    <div>
                        ...
                        <select name="color"
                            ...
                            <option value="warning" {{ old('color') == 'warning' ? 'selected' : '' }}>Warning (Orange)</option>
                            <option value="danger" {{ old('color') == 'danger' ? 'selected' : '' }}>Danger (Red)</option>
                            <option value="info" {{ old('color') == 'info' ? 'selected' : '' }}>Info (Sky)</option>
                        </select>
                    </div>
